...
public class TerminalConcurrentAsyncStreams {
    
    actor Progress {
        var streams: [String: Int] = [:]
        
        func update(forStream streamId: String, progress: Int) {
            self.streams[streamId] = progress
        }
    ...
    public static func render(title: String, completionMessage: String, theme: Theme, asyncStreams: [String: AsyncThrowingStream<Int, Error>], streams: StandardPipelines = StandardPipelines()) async throws  {
        if isTerminalInteractive() {
            try await renderInteractive(title: title, completionMessage: completionMessage, theme: theme, asyncStreams: asyncStreams, streams: streams)
        } else {
            // TODO
        }
    }
    
    public static func renderInteractive(title: String, completionMessage: String, theme: Theme, asyncStreams: [String: AsyncThrowingStream<Int, Error>], streams: StandardPipelines = StandardPipelines()) async throws  {
        ...
        @Sendable func renderProgress() async {
            ...
            for streamId in asyncStreams.keys.sorted() {
                ...
                if progress == 100 {
                    ...
                    let progressBar = "\((0..<filledProgressCharacters).map({ _ in "◼︎"}).joined())\((0..<emptyProgressCharacters).map({ _ in "-"}).joined())"
            ...
            \( completedProcesses.isEmpty ? "" : " \n\("\(completionMessage):".hex(theme.success).bold) \(completedProcesses.joined(separator: ", "))")
            """
            } else {
            """
            \(completedProcesses.isEmpty ? "" : "\("\(completionMessage):".hex(theme.success).bold) \(completedProcesses.joined(separator: ", "))")
            """
            }
