...
public class CollapsibleStream {
    ...
    public static func render(title: String, stream: AsyncThrowingStream<Event, Error>, theme: Theme, standardPipelines: StandardPipelines = StandardPipelines()) async throws  {
        try await CollapsibleStream(title: title, stream: stream, theme: theme, standardPipelines: standardPipelines)
    }
        
    @discardableResult private init(title: String, stream: AsyncThrowingStream<Event, Error>, theme: Theme, standardPipelines: StandardPipelines = StandardPipelines()) async throws {
        try await self.render(title: title, stream: stream, theme: theme, standardPipelines: standardPipelines)
    }
    
    private func render(title: String, stream: AsyncThrowingStream<Event, Error>, theme: Theme, standardPipelines: StandardPipelines) async throws {
    ...
    private func renderNonInteractive(title: String, stream: AsyncThrowingStream<Event, Error>, theme: Theme, standardPipelines: StandardPipelines) async throws {
    ...
    private func renderInteractive(title: String, stream: AsyncThrowingStream<Event, Error>, theme: Theme, standardPipelines: StandardPipelines) async throws {
        ...
        await renderer.render("\(formatCompletedPrefix("Completed: ", theme: theme))\(title)", standardPipeline: standardPipelines.output)
    }
