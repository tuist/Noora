  ...
  return (env.JAEGER ?? mockJaegerBinding()).enterSpan("add_headers", (span) => {
    ...
    return matches.forEach(({ set = {}, unset = [] }) => {
      unset.forEach((key) => {
        ...
        setMap.has(key.toLowerCase()) ? (response.headers.append(key, set[key]), span.addLogs({ append_header: key })) : (response.headers.set(key, set[key]), setMap.add(key.toLowerCase()), span.addLogs({ add_header: key }));
      });
    }), response;
  });
}
