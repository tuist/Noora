...
struct TextPrompt {
    ...
    func run() -> String {
        ...
        var currentErrors: [ValidatableError] = []

        while true {
            var input = ""

            terminal.withoutCursor {
                render(input: input, errors: currentErrors)
                while let character = terminal.readCharacter(), !isReturn(character) {
                        ...
                        let isBackspace = character.unicodeScalars.first?.value == 8 ||
                            character.unicodeScalars.first?.value == 127
                    #else
